...
const LeadForm = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!alreadyOperatesValid || !phoneValid || !nameValid || isSubmitting) {
      return;
    }
    ...
    try {
      // libera o site imediatamente, sem depender do webhook
      localStorage.setItem('lead_captured', 'true');
      localStorage.setItem('timer_expiry', (Date.now() + 600000).toString());
      onComplete();

      // envia os dados em paralelo para o Google Sheets
      ...
      if (webhookUrl) {
        ...
        console.warn('Webhook do Google Sheets não configurado.');
      }
    } catch (error) {
      console.error('Erro ao concluir envio do formulário:', error);

      // fallback extra: ainda libera o site mesmo se algo falhar
      localStorage.setItem('lead_captured', 'true');
      localStorage.setItem('timer_expiry', (Date.now() + 600000).toString());
      onComplete();
    } finally {
    ...
    <div className="fixed inset-0 z-[100] flex items-start md:items-center justify-center bg-zinc-950 px-6 py-10 overflow-y-auto">
      ...
      >
        <div className="text-center mb-6 md:mb-8" translate="no">
          ...
          <h2 className="text-lg md:text-2xl font-black text-white mb-4 leading-tight">
            Você está a um passo de garantir sua condição especial
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
          <div>
            <label className="block text-[11px] md:text-sm font-medium text-zinc-400 mb-1 ml-1">
              Nome Completo
            </label>
          ...
          <div>
            ...
            {errors.alreadyOperates && (
              <p className="text-red-400 text-[10px] mt-1 ml-1">{errors.alreadyOperates}</p>
            )}
          </div>
      ...
      {isAuthorized && (
        <main className="relative">
                ...
                <div className="lg:w-1/2">
                  ...
                  >
                    ...
                    <p className="text-xl text-zinc-400 mb-8 leading-relaxed">
                      Visualize o mercado como os profissionais. Nossa planilha exclusiva entrega os pontos exatos de entrada.
                    </p>
          ...
          <section className="py-24 bg-zinc-950 relative overflow-hidden">
            <div className="container mx-auto px-6 relative z-10">
              <SectionTitle light subtitle="Acompanhe a leitura profissional do mercado em tempo real com nossas ferramentas exclusivas.">
